...
Options for install:
  ...
  -S, --setups                   Setup permissions/services
  -F, --files                    Copy config files only
  -f, --force                    (Dangerous) Force mode without any confirm
  -c, --clean                    Clean the build cache first
      --fisrtrun                 Act like it is the first run
...
para=$(getopt \
  -o hDSFk:fc \
  -l deps,files,setups,help,force,firstrun,fontset:,clean,skip-allgreeting,skip-alldeps,skip-allsetups,skip-allfiles,ignore-outdate,skip-sysupdate,skip-plasmaintg,skip-backup,skip-quickshell,skip-fish,skip-hyprland,skip-fontconfig,skip-miscconf,core,exp-files,via-nix \
  -n "$0" -- "$@")
...
while true ; do
  case "$1" in
    ## Subcommands (short flags, before options)
    -D|--deps) SUB_SUBCMD="deps"; shift;;
    -S|--setups) SUB_SUBCMD="setups"; shift;;
    -F|--files) SUB_SUBCMD="files"; shift;;
    ## Already processed in phase 1, but not exited
    -c|--clean) shift;;
    ## Ones without parameter
    -f|--force) ask=false;shift;;
    --firstrun) INSTALL_FIRSTRUN=true;shift;;
